...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        String outcome;
        for(int i=0; i<3; i++){
            outcome = checkColumn(grid, i);
            if(!result.equals(outcome)){
                return outcome;
            }
            outcome = checkRow(grid, i);
            if(!result.equals(outcome)){
                return outcome;
            }
        }
        outcome = checkDiagonals(grid);
        if(!result.equals(outcome)){
            return outcome;
        }
    ...
    private String checkColumn(char [][]grid, int column){
        if(grid[column][0] == grid[column][1] && grid[column][0] == grid[column][2]){
            if(grid[column][0] == 'o'){
                return "O wins";
            }
            else if(grid[column][0] == 'x'){
                return "X wins";
            }
        }
        return "None";
    ...
    private String checkRow(char [][]grid, int row){
        if(grid[0][row] == grid[1][row] && grid[0][row] == grid[2][row]){
            if(grid[0][row] == 'o'){
                return "O wins";
            }
            else if(grid[0][row] == 'x'){
                return "X wins";
            }
        }
        return "None";
    ...
    private String checkDiagonals(char [][]grid){
    ...
    private String checkTie(char [][]grid){
        for(int i=0; i<3; i++)
            for(int j=0; j<3; j++){
                if(grid[i][j]=='-'){
                    return "None";
